...
drLapply <- function(X, FUN, combine = combDdo(), output = NULL, overwrite = FALSE, params = NULL, control = NULL, verbose = TRUE) {
   if(!is.function(FUN))
      stop("FUN must be a function")
   tmp <- addTransform(X, FUN)      
   
   recombine(data = tmp, combine, output = output, overwrite = overwrite, params = params, control = control, verbose = verbose)
}
